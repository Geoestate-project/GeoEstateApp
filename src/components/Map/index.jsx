...
const MapComponent = ({ properties, updateProperties }) => {
  ...
  const focusOnProperty = (property) => {
    ...
    if (mapRef.current && property.latitud && property.longitud) {
      const position = new window.google.maps.LatLng(property.latitud, property.longitud);
      mapRef.current.panTo(position);
      mapRef.current.setZoom(15); // Zoom in when focusing
      
      // Add a slight bounce effect
      setTimeout(() => {
        mapRef.current.panBy(0, -50); // Move up slightly
        setTimeout(() => {
...
<div style={{
  ...
  <div style={{
    ...
    <h5 style={{ margin: 0 }}>Filtered Properties ({filteredProperties.length})</h5>
    <button 
  ...
  <div style={{
    ...
    {filteredProperties.map((property, index) => (
      ...
      style={{
        ...
        cursor: 'pointer',
        backgroundColor: selectedProperty?.idPropiedad === property.idPropiedad 
        ? '#e6f2ff' 
        : 'white',
      borderLeft: selectedProperty?.idPropiedad === property.idPropiedad 
        ? '4px solid #4d90fe' 
        : '4px solid transparent',
        transition: 'all 0.2s', // Changed from 'background-color' to 'all'
        transform: selectedProperty?.idPropiedad === property.idPropiedad 
        ? 'scale(1.02)' 
        : 'scale(1)',
      boxShadow: selectedProperty?.idPropiedad === property.idPropiedad
                  ...
                  (property, index) =>
                    ...
                    property.longitud && (
                      <CustomMarker
                        ...
                        isFocused={focusedProperty?.idPropiedad === property.idPropiedad}
                      />
                    )
                )}
